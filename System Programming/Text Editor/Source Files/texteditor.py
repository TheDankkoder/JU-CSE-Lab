...
main_application.geometry('1500x700')
main_application.title("Text Editor - System Programming A2 - 40")
...
# for i in fo:
#     print(i, fo[i])
# print(font_size['values'].index('20'))
font_size.current(font_size['values'].index('20'))
...
#################### this statement is used to print default values of font
...
def change_font_color():
    ...
    ############ will print tuples of rgb and hexa values of color we chose
    # print(color_var)
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        #########-------------- if you want to include spaces as characters
        # characters = len(text_editor.get(1.0, 'end-1c').replace(" ",""))
        #########--------------------- if you dont want to include spaces as characters
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            ...
            if mbox is True:
                if url: #if the file exist before
                    content = str(text_editor.get(1.0, tk.END))
                    with open(url, 'w', encoding='utf-8') as fw:
                        fw.write(content)
                        main_application.destroy()
                else:# if we create a new file for saving the file
...
def hide_toolbar():
    ...
    if show_toolbar:
        ...
        tool_bar.pack(side=tk.TOP, fill=tk.X)
        text_editor.pack(fill=tk.BOTH, expand=True)
        status_bar.pack(side=tk.BOTTOM) ######## becoz of this line when we uncheck the tool bar and status bar then if we check the tool bar then status bar will also appear becoz we are resetting the whole window.
        show_toolbar = True 
